...
function PagePanel({ page, onOpen }: { page: DesktopPage; onOpen: (page: DesktopPage) => void }) {
  ...
  return <section className="desktop-page"><p className="eyebrow">Contact</p><h1>Let&apos;s make something useful.</h1><p>The contact form and mail delivery will be wired once the sending address is set.</p><LiquidMetalButton label="Back home" onClick={() => onOpen("home")} /></section>;
}
      ...
      {windowState === "open" && (
        <section className={`portfolio-window ${maximized ? "window-maximized" : ""}`} aria-label="Labish's portfolio desktop">
          ...
          <div className="window-scroll">
            <div className="window-content"><PagePanel page={page} onOpen={openPage} /></div>
